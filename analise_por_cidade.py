...
for dset in q_is:
    ...
    for _, city_row in municipios.iterrows():
        ...
        geometria_cidade = city_row.geometry  
 ...
 # Para cada UC que intersecta a cidade, calcular a área da sobreposição e atribuí-la
        ...
        latlong_km=111*110.98#111km * 111km cos(2) for para 
        resultados_cidade[nome_cidade].update({
            f"Area Cidade": latlong_km*area_cidade,
            f"Car Registros":len(car_na_cidade),
            f"CAR - Area": latlong_km*area_rural_na_cidade,
            f"Percentual CAR": percent_cidade_rural,
            f"{dset} - Area CAT ":latlong_km* area_categoria_ds_na_cidade,  # Adicionando a área da categoria ds
            f"{dset} - Area CAR sob CAT": latlong_km*area_sobreposicao_na_cidade,
...
df_resultados.to_csv('Resultados/resultados_por_cidade.csv', encoding='utf-8')
